...
export default async (req, res) => {
  ...
  const data = await fetch(apicall).then((res) => res.json());

  console.log("api/web3/etherscan: data received", data);
  ...
  if (
    result.balance != "0" &&
    tokenContractAddresses[chain][token].decimals != 18
  ) {
    for (
      let i = 0;
      i < 18 - tokenContractAddresses[chain][token].decimals;
      i++
    ) {
      result.balance += "0";
    }
